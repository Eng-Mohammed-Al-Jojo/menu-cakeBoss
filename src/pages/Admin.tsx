...
import {
  FiDownload, FiSettings, FiUpload, FiLogOut,
  FiLayout, FiDatabase, FiLock, FiMail, FiUser, FiPackage
} from "react-icons/fi";
...
export default function Admin() {
  ...
  if (!authOk) {
    return (
      <div className="admin-theme min-h-screen flex items-center justify-center bg-(--bg-main) p-6 relative overflow-hidden">
  ...
  // ================= ADMIN PANEL UI =================
  return (
    <div className="admin-theme min-h-screen bg-(--bg-main) flex justify-center py-6 sm:py-10 px-4 md:px-10">
      <div className="w-full max-w-6xl space-y-8 sm:space-y-10">
        ...
        <header className="bg-(--bg-card)/50 backdrop-blur-xl border border-(--border-color) p-6 rounded-4xl flex flex-col md:flex-row justify-between items-center gap-6 shadow-premium">
          ...
          <div className="flex items-center gap-3 flex-wrap justify-center w-full md:w-auto">
            ...
            <div className="flex items-center gap-2 bg-(--bg-main) p-1.5 rounded-2xl border border-(--border-color)">
              <button onClick={() => setShowSettings(true)} className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-primary/10 hover:text-primary text-(--text-muted) transition-all" title={t('admin.settings')}>
                <FiSettings size={20} />
              </button>
        ...
        {showSettings && (
          <OrderSettingsModal
            setShowOrderSettings={setShowSettings}
